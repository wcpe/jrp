...
- 交付 FR-12 运行日志通道（jrps 侧）：新增 `log_events` 表与有界缓冲提交口——提交永不阻塞，缓冲满时 DEBUG/INFO 丢弃并计入可观测丢弃计数，WARN/ERROR 挤掉最旧低等级事件保留；批量落库按间隔或刷写请求执行。统一脱敏辅助落地规格 §3.4 矩阵：token 摘要前缀、密码与派生材料完全隐藏、Cookie 值禁止、Authorization 只留方案类别、路径摘要掩码查询串值。
...
- 修复排空恒等满上限：控制连接的处理 goroutine 曾计入代的等待组，而客户端的控制连接在整个运行期都开着，导致每次 Apply 都要等满排水上限并一律标记为"排空异常"。控制连接跨代存活，其等待组归 Engine，排空只等数据面；实测同一用例由 3–10 秒降至 0.02 秒，且干净排空如实返回 `Stage: Drained`。
- FR-26 收窄宿主注入资源范围：`Deployment` 不再接受宿主注入的 `net.Listener`，访客入口一律由 Core 按快照自建；宿主注入与托管语义拆为独立需求（见规格 §6）。UDP 入口的会话参数在构造期固定，因此"参数变了但端口没变"按 `Stage: Prepare` 明确拒绝并说明原因，而不是表现为与原因无关的端口占用失败。
- FR-26 测试与变异验证：新增入口复用、新增代理保留已有入口、UDP 参数变更拒绝、UDP 换端口释放旧入口等端到端用例，并对客户端侧 Apply 的 drain 语义、单飞、revision 比较、幂等、失败释放、生命周期校验、绑定拒绝共七处逐一做变异验证（确认删掉实现分支后测试变红）。
