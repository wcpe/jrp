...
- 交付 FR-15 通知的**发送侧**：jrps 新增 `internal/notify` 渠道层，支持 Webhook 与 SMTP 邮件两种渠道，投递由事务 outbox 在业务事务提交后触发，外部副作用不在事务内发生（ADR-0004、FR-15 §3.2）。**已知未接线**：outbox 的写入侧尚无生产调用方——产生通知的业务动作（配置应用结果、代理变更）属 FR-10 与 FR-11，尚未交付，故 `notification_outbox` 当前没有任何生产路径写入它。派发、渠道、重试与生命周期均已就绪，接线随对应业务动作交付补齐。
