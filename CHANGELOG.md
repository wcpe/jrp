...
- FR-16 明确未交付范围：依赖 FR-13 正文采集的 4 条验收（清理双限制生效、孤儿段回收审计、正文清理后审计仍在、策略放宽不恢复）与正文查看/导出/删除的审计写入，均待 FR-13 落地后验收，当前不视为通过。
...
- FR-15 的实机验收未完成：规格 §5 要求在真实环境配置受控 Webhook 接收端与真实 SMTP 目标，并验证重试退避与 Web 通知页展示。该项需在实机执行，因此不随本批标记交付。
