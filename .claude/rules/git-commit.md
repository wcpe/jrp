...
### 2.1 禁止引用 commit hash

提交信息、文档、规则与协议契约中**不得引用本仓库的 commit hash**（含缩写与 `a..b` 区间）。

理由：hash 随历史改写、rebase 与 squash 而失效，且对读者无信息量——它指向的内容无法从字面理解，读者必须离仓查证。需要指代历史提交时用可自解释的文字，例如“上一批五个提交”“FR-15 发送侧提交”。

**例外**：外部参考仓库的兼容基线 hash 可以保留，但必须就近标明它是外部仓库基线而非本仓库提交。当前唯一例外是 ADR-0010 固定的官方 frp 基线 `e1f1d6ab1b591ec9b596f4b1b6a81cd2960c6314`，用于替代本机检出路径以保证文档可移植；它不属于本仓库（`git cat-file -t` 查不到）。新增同类例外须在 ADR 中登记理由。
...
6. 检查暂存区与提交信息不含本仓库 commit hash；不含绝对路径、真实凭据与内网地址（见 §2.1）。
