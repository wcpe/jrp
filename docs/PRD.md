...
| FR-27 | 提供有界类型化事件订阅与只读状态快照，使嵌入宿主可实时观测并在丢事件后重建状态 | P1 | 已交付@0.3.0 |
...
| S3 | FR-26、FR-27、FR-03、FR-06b、FR-07、FR-10、FR-13、FR-12 | 8 | S2 | 进行中 | Apply 与事件订阅、日志、官方 frpc 接入、STCP/XTCP、token、无中断热更、正文采集；FR-26 已交付@0.3.0，FR-27 已交付@0.3.0（SSE 集成验证随 FR-11、吞吐容差量化随 FR-14），宿主注入网络资源拆为独立后续需求 |
...
- FR-12 的**运行日志通道**已交付并实机复证：`log_events` 表与有界缓冲提交口（提交永不阻塞，缓冲满按等级降级并计数）、统一脱敏辅助（五类敏感值写入点脱敏）、`GET /api/v1/logs`（会话 401/过滤/游标分页/查看动作写审计）、配置应用四阶段日志接线（随 FR-10 编排层）、事件适配器（订阅 FR-27 通道，把 ClientConnected/ApplyResult/EngineStopped/ResyncRequired 映射为运行日志，溢出时记录含丢弃计数的 WARN）。剩余两项：**请求日志生成路径随 FR-13**（字段模型与查询已就绪，采集关闭零输出约束已测试）；**双平台实机验收**（Windows/Linux 日志页过滤查看 + 日志文件权限核对）需用户在场执行。
- FR-27 已交付@0.3.0：事件模型与首版集合、有界缓冲订阅、按等级溢出降级与 ResyncRequired、`State()` 深复制快照全部通过自动化验收（三平台 CI、竞态、静态分析）。两条后续注记：「慢消费者不降吞吐」的容差数值随 FR-14 压测定稿补齐（不作为门禁）；SSE 管理端点属 FR-11 适配器范围，交付 `/api/v1/events` 时验证背压不回传 Core。
...
- FR-16 分四批验收，S2 批次已通过实机：审计模型与封闭枚举、`GET /api/v1/audit-events` 查询（过滤与游标分页）、`GET/PUT /api/v1/capture-policy` 策略对象。实机验证了策略默认值读取、越界值一次给全违规项、缺 CSRF 返回 403、变更可读回且留下含前后值的中文审计，并对 41 条审计逐条核对——管理员口令、通知密钥、SMTP 授权码、会话令牌与 CSRF token 六类敏感值均未出现，六项字段无缺失。
  ...
  - 随 FR-10 补验（部分）：**已通过**。配置应用的应用结果审计（`apply_failure`、`apply_publish`）已随 FR-10 编排层交付并实机验证留痕；「修改代理并应用」中的代理编辑端点属 FR-11，该动作的审计届时补验。
...
- FR-15 剩余未覆盖的写入：代理变更属 FR-11，其写入点随 FR-11 补齐。配置应用失败的写入点已随 FR-10 交付（`config_apply_failed` 事件，失败时同事务广播，成功不发通知）。
- FR-15 的**实机验收**分三批执行，接收端改用公网对端（SSRF 防护与 SMTP 同源校验都不放开，接收端端口限 443/8443/9443）：
  ...
  - 随 FR-10 补验：**已通过**。配置应用失败产生 `config_apply_failed` 事件并进入投递链路（写入点随 FR-10 交付），实机验证 5 条通知送达公网对端（httpbin.org 实收 HTTP POST）、载荷为中文脱敏摘要（含 revision、阶段与安全错误摘要），且不可达期间的退避重试与失败终态同轮复证。
