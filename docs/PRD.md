...
| FR-02 | 提供首次初始化和单管理员认证，不引入多管理员或 RBAC | P1 | 开发中 |
...
| FR-16 | 审计安全敏感操作，并执行正文默认 30 天、总量 5 GiB 的保留策略 | P1 | 开发中 |
...
- S2 的 FR-25 已实现并通过功能验收，其规格任务项与状态已登记为「已交付@0.1.0」；S2 其余需求仍按切片整体推进。
- FR-12 依赖 FR-27 提供的有界事件通道，故与 FR-27 同属 S3；其日志等级、脱敏矩阵与查询接口本身不依赖 FR-27。
- FR-16 分两批验收：审计模型与封闭枚举、写入点覆盖、`GET /api/v1/audit-events` 查询、`GET/PUT /api/v1/capture-policy` 策略对象、按时间维度的审计保留清理已实现；其中 4 条验收依赖 FR-13 的正文采集落地（清理双限制各自生效、孤儿段回收与缺失引用写入审计、正文被清理后审计事件仍在、策略放宽后已清理正文不恢复），以及正文查看/导出/删除的审计写入，这些条目不随本批标记通过，待 FR-13 交付后一并验收。
