...
- FR-02 的**实机验收**受阻于两个前置，当前不标记交付：管理台界面属 FR-11（S4，未交付，当前 Web 前端只有首页骨架）；且 jrps 没有 HTTPS 监听实现，规格要求「用 HTTPS 访问管理台」却没有任何 FR 覆盖管理面的 TLS 终止，属规格与实现之间的缺口，需明确归属（内建 TLS 还是由部署层反向代理承担）。会话机制本身已由自动化套件与实机 API 验证：Cookie 具备 `HttpOnly`、`Secure` 与 `SameSite=Strict`，CSRF 与登出语义正确。
