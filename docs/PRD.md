...
| FR-09 | jrps 与 jrpc 分别以 SQLite 保存配置、revision 与必要运行元数据 | P1 | 计划 |
...
| FR-25 | 提供可嵌入其他 Go 应用的 ServerEngine/ClientEngine，并以 TCP、wire v1、TCP 代理形成首个真实垂直切片 | P1 | 计划 |
...
| FR-32 | Core 提供类型化配置构建器与校验 API，供嵌入宿主构造不可变配置快照；Core 不读取配置文件与环境变量 | P1 | 计划 |
...
| 切片 | 包含 FR | 条数 | 前置 | 说明 |
|---|---|---|---|---|
| S1 | FR-32、FR-04、FR-09、FR-12 | 4 | 无 | 配置构建、wire 编解码、SQLite 持久化、日志基础 |
| S2 | FR-25、FR-05a、FR-06a、FR-02、FR-15、FR-16 | 6 | S1 | Engine 门面与首个垂直切片、TCP 传输、四种代理、认证、通知、审计 |
| S3 | FR-26、FR-27、FR-03、FR-06b、FR-07、FR-10、FR-13 | 7 | S2 | Apply 与事件订阅、官方 frpc 接入、STCP/XTCP、token、无中断热更、正文采集 |
