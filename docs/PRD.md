...
| FR-15 | 支持 Webhook 与邮件通知，通知副作用在事务提交后触发 | P1 | 开发中 |
...
- FR-16 分两批验收：审计模型与封闭枚举、`GET /api/v1/audit-events` 查询、`GET/PUT /api/v1/capture-policy` 策略对象、按时间维度的审计保留清理已实现；审计写入点覆盖了认证、代理与配置变更、应用结果、保留策略与通知目标管理。**尚未覆盖的写入点如下，均依赖其他 FR**：
  - 依赖 FR-13（正文采集）：4 条验收（清理双限制各自生效、孤儿段回收与缺失引用写入审计、正文被清理后审计事件仍在、策略放宽后已清理正文不恢复）、正文查看/导出/删除的审计写入，以及「审计内容不含完整正文」这一敏感值类别的断言。
  - 依赖 FR-07（token 管理）：`client_rotate` 与 `client_revoke` 两个动作已在封闭枚举中登记，但**尚无写入点**——FR-07 未交付前不存在轮换与吊销动作本身。
  - 依赖 FR-12（日志）、FR-11（Web 界面）：日志查询的审计写入点未定义；「策略放宽后界面如实说明」中的界面部分未实现。
  - 上述条目不随本批标记通过，待对应 FR 交付后一并验收。
...
- FR-15 的**实机验收**受阻于三重前置，当前不标记交付：
  - 投递对端不可在本地构造。Webhook 与 SMTP 两条渠道的安全策略都拒绝回环与私有地址（规格 §3 的 SSRF 防护、`email.go` 的同源校验），而实机条款要求配置「本地受控 Webhook 接收端」，两者直接冲突；真实投递需公网可达的 HTTPS 接收端（端口限 443/8443/9443）与公网 SMTP 账号。该冲突需在后续决策中定夺（放开内网白名单、或改写实机条款为公网对端）。
  - 实机要求触发的「配置应用失败」与「token 轮换」分别属 FR-10 与 FR-07，尚未交付。
  - 「Web 通知页展示最终失败状态」属 FR-11，尚未交付。
