...
- FR-16 分四批验收，S2 批次已通过实机：审计模型与封闭枚举、`GET /api/v1/audit-events` 查询（过滤与游标分页）、`GET/PUT /api/v1/capture-policy` 策略对象。实机验证了策略默认值读取、越界值一次给全违规项、缺 CSRF 返回 403、变更可读回且留下含前后值的中文审计，并对 41 条审计逐条核对——管理员口令、通知密钥、SMTP 授权码、会话令牌与 CSRF token 六类敏感值均未出现，六项字段无缺失。
  - 随 FR-07 与 FR-10 补验：创建客户端并轮换 token、修改代理并应用各自产生的审计事件；`client_rotate` 与 `client_revoke` 已在封闭枚举中登记，但**尚无写入点**——对应 FR 未交付前不存在这两个动作本身。
...
- FR-15 的**实机验收**分三批执行，接收端改用公网对端（SSRF 防护与 SMTP 同源校验都不放开，接收端端口限 443/8443/9443）：
  ...
  - 随 FR-07 与 FR-10 补验：触发一次配置应用失败与一次 token 轮换，确认对应的两条通知依次到达。这两个触发源属 S3，在对应 FR 交付前不存在。
