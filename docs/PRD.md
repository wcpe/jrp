...
- FR-15 的**发送侧**已实现：事务 outbox 的派发与提交后发送机制、渠道层（Webhook 与 SMTP 邮件）、SSRF 防护、重试与退避抖动、租约回收、失败终态与 discarded 路径、目标 CRUD 与测试通知端点、后台发送循环随进程优雅启停。**但 outbox 缺少生产可达的写入点**：产生通知的业务动作（配置应用结果、代理变更）分别属 FR-10 与 FR-11，两者尚未交付，故 `notification_outbox` 当前没有任何生产路径写入它，规格 §5「业务动作成功后同一事务写入 outbox」由测试自造业务动作满足，不计为通过。此外实机验收（真实 Webhook 接收端与 SMTP 目标、重试退避、Web 通知页展示）需用户参与。以上两项完成前 FR-15 不标记交付。
